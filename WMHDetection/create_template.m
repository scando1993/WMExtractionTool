function [ output_args ] = create_template( route, data_folder )
...
count = 1;
% rc2 = zeros(length(subjects(:,1)),1);
for i = 1: length(subjects(:,1))
    fnm = spm_select('List', fullfile(data_dir, deblank(subjects(i,:))),'^rc2sT.*\.nii$'); % select images for the smooth
    rc2{count,1} = fullfile(data_dir, deblank(subjects(i,:)), deblank(fnm(1,:)));
    count = count + 1;
end
