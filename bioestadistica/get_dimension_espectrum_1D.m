...
function [D0,D1,D2,L,V,RD1] = get_dimension_espectrum_1D(object,r)
...
for index_size = 1:numel(r)
    ...
    N = nan(width_i/cell_size,1); %counting elements grid of the image with size depending on cell_size
    ...
    if (cell_size == 1) %max resolution: box is pixel-size
        ...
        for i=1:cell_size:width_i-cell_size+1    
            %for j=1:cell_size:width_j-cell_size+1
                ...
                current_cell = object(i:i+cell_size-1);
                ...
                N(1+((i-1)/cell_size)) = current_N; %store N value
